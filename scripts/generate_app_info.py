...
def get_major_minor_rev():
    proc = subprocess.run(['git', 'tag'], check=True, capture_output=True, text=True)
    tags = proc.stdout.strip().splitlines()
    pattern = re.compile('^\d+\.\d+$')
    minor_tags = [tag for tag in tags if pattern.search(tag)]
    minor_tag = minor_tags[-1]

    proc = subprocess.run(['git', 'rev-list', f'{minor_tag}..', '--count'], check=True, capture_output=True, text=True)
    commit_count_since_tag = proc.stdout.strip()

    return f'{minor_tag}.{commit_count_since_tag}'
...
def generate_version():
    ...
    if os.environ.get('DCMEDIT_RELEASE') != 'true':
        version += '-local'
    elif (branch := get_branch()) != 'master':
        version += f'-{branch}'
