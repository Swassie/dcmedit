...
void Split_presenter::show_default_layout() {
    m_presenters.clear();
...
void Split_presenter::switch_to_dataset_view(IPresenter& presenter) {
    auto vp_pair = m_view_factory.make_dataset_view();
    replace_view(std::move(vp_pair), presenter);
}

void Split_presenter::switch_to_image_view(IPresenter& presenter) {
    auto vp_pair = m_view_factory.make_image_view();
    replace_view(std::move(vp_pair), presenter);
}

void Split_presenter::replace_view(VP_pair vp_pair, IPresenter& presenter) {
    for(size_t i = 0; i < m_presenters.size(); i++) {
        if(m_presenters[i].get() == &presenter) {
            setup_event_handlers(*vp_pair.first, *vp_pair.second);
